...
const ASTRO_HISTORY_TTL_MS=24*60*60*1000;
const ASTRO_PRODUCTS=[
  {id:'birth-basic',name:'출생차트 성향 분석',price:augustPromoPrice(1500),desc:'성격, 기질, 감정 패턴처럼 본인의 기본 구조를 중심으로 보는 풀이'},
  {id:'love-marriage',name:'연애/결혼운',price:augustPromoPrice(1500),desc:'금성, 화성, 달, 7하우스 흐름을 중심으로 보는 관계 풀이'},
  {id:'career-money',name:'직업/재물운',price:augustPromoPrice(1500),desc:'태양, 수성, 목성, 토성, 2·10하우스 흐름을 보는 현실 풀이'},
  {id:'yearly',name:'올해 운세',price:augustPromoPrice(1500),desc:'현재 트랜짓과 출생차트의 주요 각도를 보는 연간 풀이'},
  {id:'full-report',name:'종합 점성술 리포트',price:augustPromoPrice(2500),desc:'출생차트와 현재 운을 종합해 깊게 보는 프리미엄 리포트'}
];
const ASTRO_CITIES=[
...
function astroApplyLanguage(scope){
  try{
    if(scope&&typeof currentLanguage==='function'&&currentLanguage()!=='ko'){
      if(typeof scheduleLanguageApply==='function')scheduleLanguageApply(scope);
      else if(typeof applyLanguageToVisibleResult==='function')setTimeout(()=>{try{applyLanguageToVisibleResult(scope);}catch(e){}},0);
    }
  }catch(e){}
}
...
function renderAstrologyHome(root){
  ...
  root.innerHTML=`<section class="astro-hero"><div class="astro-kicker">Astrology</div><div class="astro-title">정확한 계산으로 차트를 먼저 세워요</div><div class="astro-desc">행성 위치는 천문 계산 엔진으로 계산합니다. 무료 오늘의 점성술은 계산표로 확인하고, 유료 풀이는 계산 데이터를 바탕으로 더 깊게 정리해요.</div></section>
  <div class="astro-menu">
    ...
    <button class="astro-card" onclick="astroNavigate('input')"><span class="material-symbols-outlined astro-icon">orbit</span><div class="astro-section-title">점성술</div><div class="astro-text">출생차트를 계산한 뒤 유료 상품별 상세 리포트를 받아요.</div><span class="astro-badge">${augustPromoPriceLabel('1,500원부터')}</span></button>
    <button class="astro-card" onclick="astroNavigate('history')"><span class="material-symbols-outlined astro-icon">history</span><div class="astro-section-title">점성술 히스토리</div><div class="astro-text">저장된 유료 풀이를 다시 확인해요.</div></button>
  </div>`;
}
...
function renderAstrologyInput(root,todayOnly=false,natalOnly=false){
  astroSetHeader(todayOnly?'오늘의 점성술':natalOnly?'출생차트':'출생정보 입력',todayOnly?'무료 계산 풀이':natalOnly?'무료 네이탈 차트':'정확한 차트 계산을 위한 정보');
...
function submitAstrologyBirth(){try{const birthInfo=readAstroBirthForm();const chart=calculateAstrologyChart(birthInfo);_astroFlow={birthInfo,chart,transit:calculateTransit(new Date()),createdAt:new Date().toISOString()};saveAstrologyFlow();astroNavigate('products');}catch(e){alert(e.message||'차트 계산 중 오류가 발생했어요.');}}
...
function renderTodayAstrologyResult(root){
  astroSetHeader('오늘의 점성술','무료 결과');
  const chart=_astroFlow.chart,transit=_astroFlow.transit,text=_astroFlow.todayText||generateTodayAstrology(chart,transit);
  const paras=String(text||'').split(/\n\s*\n/).map(readingPlainText).filter(Boolean);
  const sun=findPlanet(chart,'sun'),moon=findPlanet(chart,'moon'),todayMoon=(transit.planets||[]).find(p=>p.planet==='moon');
  const elementKo={fire:'불',earth:'흙',air:'바람',water:'물'},total=Math.max(1,Object.values(chart.elementBalance||{}).reduce((a,b)=>a+b,0));
  const dominant=getDominantElement(chart.elementBalance),todaySign=todayMoon?signById(todayMoon.sign):signById(moon.sign);
  const labels=['오늘의 큰 흐름','마음과 컨디션','사랑과 관계','일과 행동','오늘의 비책'];
  const icons=['auto_awesome','self_improvement','favorite','business_center','task_alt'];
  const quotes=['오늘의 별은 본인의 평소 방식에 새로운 온도를 더해요','감정을 억지로 정리하기보다 먼저 알아차리세요','상대의 뜻을 정하기 전에 편하게 물어보세요','하나를 선택해 현실의 결과로 남기세요','오늘 한 가지를 작게 실천해보세요'];
  const fallback=[todaySign.personality,signById(moon.sign).personality,signById(findPlanet(chart,'venus').sign).loveStyle,signById(findPlanet(chart,'mars').sign).careerStyle,'오늘의 흐름은 가능성을 보여주는 지도예요. 실제 방향은 본인의 작은 선택이 만들어요.'];
  const deck=labels.map((title,i)=>({key:`today-${i}`,icon:icons[i],title,quote:quotes[i],copy:`${paras[i]||fallback[i]} 오늘의 별 흐름은 확정된 예언이 아니에요. 본인의 현실 일정과 마음을 함께 보며 활용하세요.`,points:[{label:'오늘의 달',value:todaySign.nameKo},{label:'본인의 태양',value:signNameKo(sun.sign)}]}));
  const warning=chart.birthInfo.birthTimeUnknown?'<div class="astro-warn">출생시간을 몰라 상승궁과 하우스 풀이는 제외했어요. 달 위치는 생일 경계에 가까우면 오차가 있을 수 있어요.</div>':'';
  const libraryHtml=`${warning}${todayAstrologyVisualHTML(chart,transit)}<section class="astro-panel"><div class="astro-section-title">오늘의 상세 풀이</div><div class="astro-markdown">${markdownToAstroHTML(typeof preventOrphan==='function'?preventOrphan(text):text)}</div></section><div class="astro-actions"><button class="astro-btn" onclick="astroNavigate('products')">상세 풀이 보기</button><button class="astro-btn secondary" onclick="astroNavigate('today')">정보 다시 입력</button></div>`;
  root.innerHTML=buildStoryExperienceHTML({id:'free-today-astrology',kicker:'TODAY COSMOS',icon:'orbit',title:'오늘의 점성술',subtitle:`${chart.birthInfo.city} · 오늘의 달은 ${todaySign.nameKo}를 지나가요`,personalNote:paras[0]||fallback[0],stats:Object.entries(chart.elementBalance||{}).map(([key,value])=>({label:elementKo[key],value:Math.round(value/total*100),display:value})),highlights:[{icon:'nightlight',label:'오늘의 달',text:todaySign.nameKo},{icon:'light_mode',label:'본인의 태양',text:signNameKo(sun.sign)},{icon:'auto_awesome',label:'강한 원소',text:elementKo[dominant]}],deck,deckTitle:'오늘의 별 흐름 5장',libraryTitle:'오늘의 점성술 전체 서고',libraryIntro:'출생차트와 오늘의 행성 흐름을 겹쳐 계산한 근거와 상세 풀이를 모았어요.',libraryHtml:typeof preventOrphan==='function'?preventOrphan(libraryHtml):libraryHtml});
}
function renderAstrologySummary(root){if(!_astroFlow||!_astroFlow.chart)return astroNavigate('input');astroSetHeader('차트 요약','계산 결과 확인');const c=_astroFlow.chart;root.innerHTML=`<section class="astro-hero"><div class="astro-kicker">Calculated Chart</div><div class="astro-title">출생차트 계산 완료</div><div class="astro-desc">상세 풀이 전에 계산 결과를 먼저 확인해주세요.</div></section>${c.validationWarnings?.length?`<div class="astro-warn">${c.validationWarnings.map(astroEsc).join('<br>')}</div>`:''}<section class="astro-panel"><div class="astro-section-title">주요 포인트</div><div class="astro-chart-grid">${c.planets.map(p=>`<div class="astro-chip-card"><b>${astroEsc(planetNameKo(p.planet))}</b>${astroEsc(signNameKo(p.sign))} ${p.degree.toFixed(1)}°${p.house?` · ${p.house}하우스`:''}${p.retrograde?' · 역행':''}</div>`).join('')}${c.ascendant?`<div class="astro-chip-card"><b>ASC</b>${astroEsc(signNameKo(c.ascendant.sign))} ${c.ascendant.degree.toFixed(1)}°</div>`:''}${c.midheaven?`<div class="astro-chip-card"><b>MC</b>${astroEsc(signNameKo(c.midheaven.sign))} ${c.midheaven.degree.toFixed(1)}°</div>`:''}</div></section><section class="astro-panel"><div class="astro-section-title">무료 요약</div><div class="astro-text">${astroEsc(generateTemplateAstrologySummary(c))}</div></section><div class="astro-actions"><button class="astro-btn" onclick="astroNavigate('products')">유료 풀이 선택</button><button class="astro-btn secondary" onclick="astroNavigate('today')">오늘의 점성술 보기</button></div>`;}
function renderAstrologyProductSummary(chart){
  const sun=chart.planets.find(p=>p.planet==='sun'),moon=chart.planets.find(p=>p.planet==='moon'),venus=chart.planets.find(p=>p.planet==='venus'),mars=chart.planets.find(p=>p.planet==='mars');
  const chips=[sun&&`태양 ${formatSignName(sun.sign)}`,moon&&`달 ${formatSignName(moon.sign)}`,chart.ascendant&&`ASC ${formatSignName(chart.ascendant.sign)}`,venus&&`금성 ${formatSignName(venus.sign)}`,mars&&`화성 ${formatSignName(mars.sign)}`].filter(Boolean);
  return `<section class="astro-panel"><div class="astro-section-title">무료 요약</div><div class="astro-chart-grid">${chips.map(c=>`<div class="astro-chip-card">${astroEsc(c)}</div>`).join('')}</div><div class="astro-text" style="margin-top:12px">${astroEsc(generateTemplateAstrologySummary(chart))}</div>${chart.validationWarnings?.length?`<div class="astro-warn" style="margin-top:12px">${chart.validationWarnings.map(astroEsc).join('<br>')}</div>`:''}</section>`;
}
function renderAstrologyProducts(root){if(!_astroFlow||!_astroFlow.chart)return astroNavigate('input');astroSetHeader('점성술 상품','리포트 선택');root.innerHTML=`<section class="astro-hero"><div class="astro-kicker">Products</div><div class="astro-title">원하는 풀이를 선택하세요</div><div class="astro-desc">계산된 출생차트의 무료 요약을 먼저 확인하고, 필요한 풀이만 선택하세요.</div></section>${renderAstrologyProductSummary(_astroFlow.chart)}<div class="astro-products">${ASTRO_PRODUCTS.map(p=>`<button class="astro-product" onclick="selectAstrologyProduct('${p.id}')"><span class="astro-badge">${p.id==='full-report'?'프리미엄':'유료'}</span><div class="astro-product-title">${astroEsc(p.name)}</div><div class="astro-text">${astroEsc(p.desc)}</div><div class="astro-price">${p.price.toLocaleString()}원</div></button>`).join('')}</div><div class="astro-actions"><button class="astro-btn secondary" onclick="astroNavigate('input')">출생정보 다시 입력</button></div>`;}
...
function astroPaymentCategory(productType){return productType?`astro-${productType}`:'';}
function renderAstrologyPayment(root){if(!_astroFlow||!_astroFlow.productType)return astroNavigate('products');astroSetHeader('결제','점성술');const cat=astroPaymentCategory(_astroFlow.productType);const p=ASTRO_PRODUCTS.find(x=>x.id===_astroFlow.productType);const currentPrice=p?p.price:(_astroFlow.price||0);_astroFlow.price=currentPrice;root.innerHTML=`<section class="astro-panel"><button type="button" class="paid-secret-lock astro-secret-lock" onclick="_secretUnlockTap('${cat}')" aria-label="관리자 잠금 해제 코드 입력" title="관리자 잠금 해제"><span class="material-symbols-outlined" aria-hidden="true">lock</span></button><div class="astro-section-title">${astroEsc(_astroFlow.productName)}</div><div class="astro-text">${astroEsc(_astroFlow.birthInfo.city)} · ${astroEsc(_astroFlow.birthInfo.timezone)}</div><div class="astro-price">${currentPrice.toLocaleString()}원</div><div class="astro-text">결제가 완료되면 계산된 출생차트와 현재 트랜짓 데이터를 바탕으로 상세 풀이를 정리해요. 결과는 결제 시점부터 24시간 동안 히스토리에서 다시 볼 수 있어요.</div><div class="pay-preview-mini"><b>결제 후 바로 제공돼요</b><span>출생차트 핵심</span><span>현재 행성 흐름</span><span>관계·일·돈 조언</span><span>24시간 재조회</span></div><div class="astro-actions"><button class="astro-btn" onclick="startAstrologyPayment()">${currentPrice.toLocaleString()}원 결제하기</button><button class="astro-btn secondary" onclick="astroNavigate('products')">상품 다시 선택</button></div></section>`;}
function startAstrologyPayment(){if(!_astroFlow||!_astroFlow.productType)return;saveAstrologyFlow();const cat=astroPaymentCategory(_astroFlow.productType);if(typeof openPaymentConsent==='function'&&typeof PAID_META!=='undefined'&&PAID_META[cat])openPaymentConsent(cat,'KAKAOPAY');else alert('결제 준비 중이에요. 잠시 후 다시 시도해 주세요.');}
function completeAstrologyPayment(){const now=Date.now();_astroFlow.paymentStatus='paid';_astroFlow.paidAt=_astroFlow.paidAt||now;_astroFlow.expiresAt=_astroFlow.expiresAt||now+ASTRO_HISTORY_TTL_MS;saveAstrologyFlow();finishAstrologyReading();}
function resumePaidAstrologyAfterPayment(category){const productType=String(category||'').replace(/^astro-/,'');const p=ASTRO_PRODUCTS.find(x=>x.id===productType);_astroFlow=loadAstrologyFlow();if(!_astroFlow||!_astroFlow.chart){alert('출생차트 정보가 만료됐어요. 출생정보를 다시 입력해 주세요.');goToAstrology('input');return;}const now=Date.now();_astroFlow={..._astroFlow,productType:p?p.id:productType,productName:p?p.name:(_astroFlow.productName||'점성술 풀이'),price:p?p.price:(_astroFlow.price||augustPromoPrice(1500)),paymentStatus:'paid',paidAt:now,expiresAt:now+ASTRO_HISTORY_TTL_MS,resultMarkdown:''};saveAstrologyFlow();goToAstrology('result');finishAstrologyReading();}
window.resumePaidAstrologyAfterPayment=resumePaidAstrologyAfterPayment;
async function finishAstrologyReading(){_astroFlow.resultLoading=true;_astroFlow.resultMarkdown='';saveAstrologyFlow();astroNavigate('result');try{_astroFlow.resultMarkdown=await generateAstrologyReading(_astroFlow.chart,_astroFlow.transit,_astroFlow.productType);}catch(e){console.warn('Astrology paid reading failed:',e&&e.message);_astroFlow.resultMarkdown=generateTemplateAstrologyReading(_astroFlow.chart,_astroFlow.productType);if(typeof currentLanguage==='function'&&currentLanguage()!=='ko'&&typeof translateSingleText==='function'){_astroFlow.resultMarkdown=await translateSingleText(_astroFlow.resultMarkdown,currentLanguage());}} _astroFlow.resultLoading=false;saveAstrologyFlow();saveAstrologyReading(_astroFlow);renderAstrology();}
function astrologyResultVisualHTML(flow){if(!flow||!flow.chart)return '';const transit=flow.transit||calculateTransit(new Date());const tj={date:transit&&transit.date?new Date(transit.date):new Date(),transit,aspects:calculateTransitAspects(flow.chart,transit)};return `<section class="astro-panel astro-today-visual"><div class="astro-section-title">풀이 기준 차트</div><div class="astro-text">바깥의 금색 행성은 출생차트, 안쪽의 파란 행성은 풀이 시점의 트랜짓이에요.</div><div class="astro-wheel-wrap compact">${renderNatalChartWheel(flow.chart,tj)}</div></section>`;}
function buildAstrologyStoryConfig(flow){
  const chart=flow.chart;
  const sun=findPlanet(chart,'sun'),moon=findPlanet(chart,'moon'),mercury=findPlanet(chart,'mercury'),venus=findPlanet(chart,'venus'),mars=findPlanet(chart,'mars');
  const sunSign=signById(sun.sign),moonSign=signById(moon.sign),mercurySign=signById(mercury.sign),venusSign=signById(venus.sign),marsSign=signById(mars.sign);
  ...
  const deck=[
    ...
    {key:'astro-moon',icon:'dark_mode',title:'감정과 회복',quote:`달 ${moonSign.nameKo}, 마음이 쉬는 방식이에요`,copy:`본인의 감정과 안정감에는 ${moonSign.personality} 힘들 때는 문제를 바로 해결하려 하기보다 먼저 마음이 편해지는 환경을 만드세요. ${moonSign.caution} 감정의 파도를 기록하면 반복되는 상황과 필요한 회복 방식이 더 빨리 보여요.`,points:[{label:'달',value:moonSign.nameKo},{label:'회복 단어',value:moonSign.keywords[1]}]},
    {key:'astro-talk',icon:'forum',title:'생각과 대화',quote:`수성 ${mercurySign.nameKo}, 생각을 꺼내는 리듬이에요`,copy:`본인은 생각을 정리하고 말할 때 ${mercurySign.personality} 중요한 대화는 머릿속에서만 완성하려 하지 말고 핵심을 짧게 적어보세요. 상대가 같은 속도로 이해할 거라 가정하지 않으면 오해가 줄어요. 말의 재능은 반복해서 설명할수록 본인의 힘이 돼요.`,points:[{label:'수성',value:mercurySign.nameKo},{label:'대화 단어',value:mercurySign.keywords[0]}]},
    {key:'astro-love',icon:'favorite',title:'사랑과 관계',quote:`금성 ${venusSign.nameKo}, 마음이 끌리는 방식이에요`,copy:`관계에서는 ${venusSign.loveStyle} 호감이 커질수록 상대의 생활 속도와 경계도 함께 살펴보세요. 원하는 것을 돌려 말하기보다 부드럽고 구체적으로 표현하는 편이 좋아요. 사랑은 차트가 정해주는 답이 아니라 두 사람이 계속 맞춰가는 선택이에요.`,points:[{label:'금성',value:venusSign.nameKo},{label:'관계 단어',value:venusSign.keywords[1]}]},
    {key:'astro-action',icon:'bolt',title:'행동과 추진력',quote:`화성 ${marsSign.nameKo}, 본인이 움직이는 방식이에요`,copy:`행동할 때는 ${marsSign.personality} 의욕이 올라온 날에는 시작만 많이 만들지 말고 하나를 끝내세요. 화가 나거나 조급할 때는 몸을 먼저 움직인 뒤 결정하면 실수가 줄어요. 추진력은 방향과 마감이 있을 때 가장 멋지게 살아나요.`,points:[{label:'화성',value:marsSign.nameKo},{label:'행동 단어',value:marsSign.keywords[0]}]},
...
function renderAstrologyResult(root){
  if(!_astroFlow)return astroNavigate('input');
  astroSetHeader('점성술 결과',_astroFlow.productName||'점성술 풀이');
  if(_astroFlow.resultLoading){root.innerHTML=`<section class="astro-panel"><div class="astro-section-title">점성술 리포트를 작성하고 있어요</div><div class="astro-text">계산된 출생차트와 트랜짓 데이터를 바탕으로 풀이를 정리 중이에요.</div></section>`;return;}
  if(!_astroFlow.resultMarkdown)return astroNavigate('products');
  root.innerHTML=typeof buildStoryExperienceHTML==='function'?buildStoryExperienceHTML(buildAstrologyStoryConfig(_astroFlow)):`${astrologyResultVisualHTML(_astroFlow)}<section class="astro-panel"><div class="astro-markdown">${markdownToAstroHTML(_astroFlow.resultMarkdown)}</div></section>`;
}
function renderAstrologyHistory(root){astroSetHeader('점성술 히스토리','24시간 보관');const list=loadAstrologyHistory();root.innerHTML=`<section class="astro-hero"><div class="astro-kicker">History</div><div class="astro-title">점성술 히스토리</div><div class="astro-desc">결제 시점부터 24시간 동안 유료 풀이 결과를 다시 볼 수 있어요.</div></section><div class="astro-grid">${list.length?list.map(r=>`<section class="astro-history-item"><div class="astro-section-title">${astroEsc(r.productName||r.productType)}</div><div class="astro-small">${astroEsc(r.birthInfo.city)} · ${new Date(r.createdAt).toLocaleString('ko-KR')}</div><div class="astro-small">만료: ${new Date(r.expiresAt||Date.parse(r.createdAt)+ASTRO_HISTORY_TTL_MS).toLocaleString('ko-KR')}</div><div class="astro-actions"><button class="astro-btn" onclick="openAstrologyHistory('${r.id}')">다시 보기</button></div></section>`).join(''):'<div class="tarot-empty">24시간 안에 저장된 점성술 결과가 없어요.</div>'}</div>`;}
function openAstrologyHistory(id){const r=loadAstrologyHistory().find(x=>x.id===id);if(!r){alert('24시간이 지나 만료된 풀이예요.');return;}_astroFlow={...r,paymentStatus:'paid'};saveAstrologyFlow();astroNavigate('result');}
...
function renderNatalChartResult(root){
  ...
  const libraryHtml=`<section class="astro-hero"><div class="astro-kicker">Natal Chart</div><div class="astro-title">본인 출생차트</div><div class="astro-desc">${formatBirthDateTime(b)}<br>${astroEsc(b.country)} ${astroEsc(b.city)}<br>${unknown?'출생시간을 모르는 상태로 계산했어요.':'출생시간까지 반영해 계산했어요.'}</div></section>
  ...
  ${astrologyReadingCTA()}`;
  const elementKo={fire:'불',earth:'흙',air:'바람',water:'물'},modeKo={cardinal:'시작',fixed:'유지',mutable:'변화'};
  const total=Math.max(1,Object.values(chart.elementBalance||{}).reduce((a,b)=>a+b,0));
  const dominant=getDominantElement(chart.elementBalance),weak=getWeakElement(chart.elementBalance),mode=getDominantModality(chart.modalityBalance);
  const sunSign=signById(sun.sign),moonSign=signById(moon.sign),venusSign=signById(venus.sign),marsSign=signById(mars.sign);
  const ascSign=unknown?null:signById(chart.ascendant.sign),strongNames=strongPlanets.map(p=>formatPlanetName(p.planet)).join(' · ');
  const deck=[
    {key:'sun',icon:'light_mode',title:'태양 · 삶의 방향',quote:`본인의 태양은 ${sunSign.nameKo}에 있어요`,copy:`${sunSign.personality} 태양은 본인이 오래간 성장하며 만들어갈 방향을 보여줘요. 다른 사람의 기대보다 본인이 보람을 느끼는 일을 기준으로 삼으세요. 오늘 한 가지—지금 키우고 싶은 능력을 한 줄로 적어보세요.`,points:[{label:'태양',value:sunSign.nameKo},{label:'핵심',value:sunSign.keywords.slice(0,2).join('·')}]},
    {key:'moon',icon:'nightlight',title:'달 · 마음의 호흡',quote:`본인의 달은 ${moonSign.nameKo}에 있어요`,copy:`${moonSign.personality} 달은 평소의 감정 반응과 편안함을 느끼는 방식을 보여줘요. 마음이 흔들릴 때는 문제를 바로 해결하려 하기보다 본인에게 편안한 환경을 먼저 만드세요. 오늘 한 가지—피로와 감정을 다른 단어로 적어보세요.`,points:[{label:'달',value:moonSign.nameKo},{label:'마음의 키워드',value:moonSign.keywords.slice(0,2).join('·')}]},
    {key:'asc',icon:'north_east',title:'첫인상과 밖으로 드러나는 방식',quote:ascSign?`본인은 ${ascSign.nameKo} 분위기로 첫인상을 만들어요`:'출생시간을 알면 첫인상을 더 세밀하게 볼 수 있어요',copy:ascSign?`${ascSign.personality} 첫 만남과 새로운 환경에서 이 분위기가 먼저 드러나요. 태양의 본질과 첫인상이 다르게 느껴진다면 어느 쪽도 틀린 것이 아니에요. 상황에 따라 다른 문을 여는 본인의 방식이에요.`:'출생시간을 몰라 첫인상과 12개 삶의 영역 풀이는 제외했어요. 태양·달·금성·화성의 위치는 그대로 확인할 수 있어요. 출생시간을 알게 되면 정보를 수정해 다시 계산해보세요.',points:[{label:'첫인상',value:ascSign?ascSign.nameKo:'출생시간 미상'},{label:'계산 범위',value:unknown?'행성 중심':'전체 차트'}]},
    {key:'love',icon:'favorite',title:'금성 · 사랑과 취향',quote:`본인의 금성은 ${venusSign.nameKo}에 있어요`,copy:`${venusSign.loveStyle} 금성은 호감을 느끼는 방식과 사랑에서 중요하게 보는 가치를 보여줘요. 설렘만 보기보다 상대와 생활의 속도가 맞는지도 함께 보세요. 오늘 한 가지—좋아하는 방식을 말로 알려보세요.`,points:[{label:'금성',value:venusSign.nameKo},{label:'사랑의 키워드',value:venusSign.keywords.slice(0,2).join('·')}]},
    {key:'action',icon:'local_fire_department',title:'화성 · 행동과 추진력',quote:`본인의 화성은 ${marsSign.nameKo}에 있어요`,copy:`${marsSign.careerStyle} 화성은 원하는 것을 향해 움직이는 방식과 화가 날 때의 반응을 보여줘요. 속도가 붙을수록 방향과 마감을 먼저 확인하세요. 오늘 한 가지—시작할 일의 끝날 모습을 한 줄로 적어보세요.`,points:[{label:'화성',value:marsSign.nameKo},{label:'행동 키워드',value:marsSign.keywords.slice(0,2).join('·')}]},
    {key:'balance',icon:'balance',title:'네 원소의 균형',quote:`${elementKo[dominant]} 원소는 잘 쓰고, ${elementKo[weak]} 원소는 일상에서 보태세요`,copy:`본인 차트에서는 ${elementKo[dominant]} 원소가 가장 두드러져요. ${elementKo[weak]} 원소는 의식적으로 보태면 균형을 잡는 데 도움이 돼요. 원소는 장점과 단점을 나누는 점수가 아니에요. 본인이 어떤 방식을 자연스럽게 쓰는지를 보여줘요.`,points:Object.entries(chart.elementBalance||{}).map(([key,value])=>({label:elementKo[key],value:`${value}`}))},
    {key:'planets',icon:'orbit',title:'강하게 드러나는 행성',quote:`${strongNames||'각 행성이 고르게'} 분위기가 자주 드러나요`,copy:`강하게 드러나는 행성은 ${strongNames||'전체 행성'}이에요. 이 결과는 개인 행성·주요 각도·차트의 중요 지점을 함께 본 보조 지표예요. 하나의 행성으로 본인 전체를 단정하지 마세요. 전체 서고에서 행성 위치와 서로의 각도를 함께 확인하세요.`,points:strongPlanets.slice(0,3).map((p,i)=>({label:`${i+1}위`,value:formatPlanetName(p.planet)}))}
  ];
  root.innerHTML=buildStoryExperienceHTML({id:'free-natal-chart',kicker:'BIRTH CHARACTER',icon:'donut_large',title:`태양 ${sunSign.nameKo} · 달 ${moonSign.nameKo}`,subtitle:'본인의 삶의 방향과 마음의 호흡을 함께 보세요',personalNote:generateTemplateAstrologySummary(chart),stats:Object.entries(chart.elementBalance||{}).map(([key,value])=>({label:elementKo[key],value:Math.round(value/total*100),display:value})),highlights:[{icon:'north_east',label:'첫인상',text:ascSign?ascSign.nameKo:'출생시간 미상'},{icon:'auto_awesome',label:'강한 원소',text:elementKo[dominant]},{icon:'sync_alt',label:'주로 움직이는 방식',text:modeKo[mode]}],deck,deckTitle:'출생차트 핵심 7장',libraryTitle:'출생차트 전체 서고',libraryIntro:'원형 차트·행성 위치·12개 삶의 영역·행성 사이의 각도와 시간 여행 도구를 모두 보관했어요.',libraryHtml:typeof preventOrphan==='function'?preventOrphan(libraryHtml):libraryHtml});
}
...
function loadAstrologyHistory(){let list=[];try{list=JSON.parse(localStorage.getItem(ASTRO_HISTORY_KEY)||'[]');}catch(e){list=[];}if(!Array.isArray(list))list=[];const now=Date.now();const valid=list.filter(r=>{const exp=Number(r.expiresAt)||((r.paidAt?Number(r.paidAt):Date.parse(r.createdAt||0))+ASTRO_HISTORY_TTL_MS);return exp>now;});if(valid.length!==list.length){try{localStorage.setItem(ASTRO_HISTORY_KEY,JSON.stringify(valid));}catch(e){}}return valid;}
function saveAstrologyReading(flow){if(!flow.resultMarkdown)return;const paidAt=Number(flow.paidAt)||Date.now();const r={id:'astro_'+Date.now(),userId:typeof getTarotUserId==='function'?getTarotUserId():'local',birthInfo:flow.birthInfo,productType:flow.productType,productName:flow.productName,price:flow.price,chart:flow.chart,transit:flow.transit,resultMarkdown:flow.resultMarkdown,paidAt,expiresAt:Number(flow.expiresAt)||paidAt+ASTRO_HISTORY_TTL_MS,createdAt:new Date().toISOString()};const list=loadAstrologyHistory().filter(x=>x.productType!==r.productType||JSON.stringify(x.birthInfo)!==JSON.stringify(r.birthInfo));list.unshift(r);localStorage.setItem(ASTRO_HISTORY_KEY,JSON.stringify(list.slice(0,30)));}
