...
const ASTRO_PRODUCTS=[
  {id:'birth-basic',name:'출생차트 성향 분석',price:1500,desc:'성격, 기질, 감정 패턴처럼 본인의 기본 구조를 중심으로 보는 풀이'},
  {id:'love-marriage',name:'연애/결혼운',price:1500,desc:'금성, 화성, 달, 7하우스 흐름을 중심으로 보는 관계 풀이'},
  {id:'career-money',name:'직업/재물운',price:1500,desc:'태양, 수성, 목성, 토성, 2·10하우스 흐름을 보는 현실 풀이'},
  {id:'yearly',name:'올해 운세',price:1500,desc:'현재 트랜짓과 출생차트의 주요 각도를 보는 연간 풀이'},
  {id:'full-report',name:'종합 점성술 리포트',price:2500,desc:'출생차트와 현재 운을 종합해 깊게 보는 프리미엄 리포트'}
];
const ASTRO_CITIES=[
...
function renderAstrologyHome(root){
  ...
  <div class="astro-menu">
    ...
    <button class="astro-card" onclick="astroNavigate('input')"><span class="material-symbols-outlined astro-icon">orbit</span><div class="astro-section-title">점성술</div><div class="astro-text">출생차트를 계산한 뒤 유료 상품별 상세 리포트를 받아요.</div><span class="astro-badge">1,500원부터</span></button>
...
function renderAstrologyPayment(root){if(!_astroFlow||!_astroFlow.productType)return astroNavigate('products');astroSetHeader('결제','점성술');const cat=astroPaymentCategory(_astroFlow.productType);root.innerHTML=`<section class="astro-panel"><button type="button" class="paid-secret-lock astro-secret-lock" onclick="_secretUnlockTap('${cat}')" aria-label="관리자 잠금 해제 코드 입력" title="관리자 잠금 해제"><span class="material-symbols-outlined" aria-hidden="true">lock</span></button><div class="astro-section-title">${astroEsc(_astroFlow.productName)}</div><div class="astro-text">${astroEsc(_astroFlow.birthInfo.city)} · ${astroEsc(_astroFlow.birthInfo.timezone)}</div><div class="astro-price">${(_astroFlow.price||0).toLocaleString()}원</div><div class="astro-text">결제가 완료되면 계산된 출생차트와 현재 트랜짓 데이터를 바탕으로 상세 풀이를 정리해요. 결과는 결제 시점부터 24시간 동안 히스토리에서 다시 볼 수 있어요.</div><div class="pay-preview-mini"><b>결제 후 바로 제공돼요</b><span>출생차트 핵심</span><span>현재 행성 흐름</span><span>관계·일·돈 조언</span><span>24시간 재조회</span></div><div class="astro-actions"><button class="astro-btn" onclick="startAstrologyPayment()">${(_astroFlow.price||0).toLocaleString()}원 결제하기</button><button class="astro-btn secondary" onclick="astroNavigate('products')">상품 다시 선택</button></div></section>`;}
