...
function renderAstrologyResult(root){if(!_astroFlow)return astroNavigate('input');astroSetHeader('점성술 결과',_astroFlow.productName||'점성술 풀이');if(_astroFlow.resultLoading){root.innerHTML=`<section class="astro-panel"><div class="astro-section-title">점성술 리포트를 작성하고 있어요</div><div class="astro-text">계산된 출생차트와 트랜짓 데이터를 바탕으로 풀이를 정리 중이에요.</div></section>`;return;}if(!_astroFlow.resultMarkdown)return astroNavigate('products');root.innerHTML=`<section class="astro-panel"><div class="astro-section-title">${astroEsc(_astroFlow.productName)}</div><div class="astro-small">${astroEsc(_astroFlow.birthInfo.city)} · ${new Date(_astroFlow.createdAt||Date.now()).toLocaleString('ko-KR')}</div></section>${astrologyResultVisualHTML(_astroFlow)}<section class="astro-panel"><div class="astro-markdown">${markdownToAstroHTML(_astroFlow.resultMarkdown)}</div></section><div class="astro-actions"><button class="astro-btn" onclick="astroNavigate('products')">다른 풀이 보기</button><button class="astro-btn secondary" onclick="astroNavigate('history')">히스토리 보기</button></div>`;}
