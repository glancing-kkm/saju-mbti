...
function renderAstrologyPayment(root){if(!_astroFlow||!_astroFlow.productType)return astroNavigate('products');astroSetHeader('결제','점성술');const cat=astroPaymentCategory(_astroFlow.productType);root.innerHTML=`<section class="astro-panel"><div class="astro-section-title">${astroEsc(_astroFlow.productName)}</div><div class="astro-text">${astroEsc(_astroFlow.birthInfo.city)} · ${astroEsc(_astroFlow.birthInfo.timezone)}</div><div class="astro-price">${(_astroFlow.price||0).toLocaleString()}원</div><div class="astro-text">결제가 완료되면 계산된 출생차트와 현재 트랜짓 데이터를 바탕으로 리딩을 생성해요. 결과는 결제 시점부터 24시간 동안 히스토리에서 다시 볼 수 있어요.</div><div class="astro-actions"><button class="astro-btn" onclick="startAstrologyPayment()">${(_astroFlow.price||0).toLocaleString()}원 결제하기</button><button class="astro-btn secondary" onclick="astroNavigate('products')">상품 다시 선택</button></div></section>`;}
